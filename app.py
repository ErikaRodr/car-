        ...
        if sheet_name == 'servico':
            ...
            for col in ['valor', 'garantia_dias', 'km_realizado', 'km_proxima_revisao']:
                if col in df.columns:
                    # Trata a conversão de garantia_dias para int (necessário no cálculo de data_vencimento)
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
            # CONVERSÃO REFORÇADA PARA DATA DE SERVIÇO E VENCIMENTO
...
def write_sheet_data(sheet_name, df_new):
    ...
    try:
        ...
        # Sobrescreve toda a aba (ponto de lentidão)
        worksheet.clear()
...
if __name__ == '__main__':
    main()Você me enviou seu código em um estado onde ele estava **funcionando antes**, mas que ainda tinha falhas de sintaxe (`U+00A0`) e a lógica original de IDs e datas.

O código que você me enviou no final dessa longa cadeia de correções **já está com o erro `U+00A0` corrigido**, mas tem a lógica de **auto-incremento de ID (inteiros)** que você queria manter e as **datas corrigidas** para o formato ISO.

### 🛑 O QUE CAUSOU O ERRO `U+00A0`?

O erro **`SyntaxError: invalid non-printable character U+00A0`** na linha 33 foi causado por um **espaço não separável** (copiado de algum editor ou navegador) na indentação da docstring.

O código abaixo é a sua **versão original**, mas com a correção do `U+00A0` e a correção das datas (que causavam o erro `Timestamp`) implementadas. Isso garante que ele funcione como você pretendia.

---

## 🛠️ Código Corrigido e Funcional (Mantendo Lógica de ID Original)

Substitua todo o seu `app.py` pelo código abaixo. Ele agora está livre de erros de sintaxe e de formatação de dados, respeitando sua estrutura de auto-incremento de IDs.
...
import gspread # Biblioteca para Google Sheets
import numpy as np 
# REMOVIDO: import uuid (Mantendo a lógica original de ID inteiro)
...
def get_full_service_data(date_start=None, date_end=None):
    ...
    # 🛑 RESTAURADO: IDs de veiculos/prestadores para o join
